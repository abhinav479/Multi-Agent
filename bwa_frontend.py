...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    ...
    try:
        ...
        for step in graph_app.stream(inputs, stream_mode="updates"):
            yield ("updates", step)
            current_state = extract_latest_state(current_state, step)
        if current_state:
...
if out:
    ...
    blog_slug = safe_slug(blog_title)
    blog_images_dir = Path("images") / blog_slug
    ...
    with tab_images:
        ...
        if not specs and not blog_images_dir.exists():
            ...
            if blog_images_dir.exists():
                files = [p for p in blog_images_dir.iterdir() if p.is_file()]
                if not files:
                    st.warning(f"`{blog_images_dir}` exists but is empty.")
                else:
                    for p in sorted(files):
                        st.image(str(p), caption=p.name, use_container_width=True)

                z = images_zip(blog_images_dir)
                if z:
                    st.download_button(
